...
class AttentionKnowledgeDistillationLoss(nn.Module):
    def __init__(self,
                 options,
                 distillation_criterion = nn.KLDivLoss(reduction='batchmean'),
                 student_criterion = nn.CrossEntropyLoss()):
        super(AttentionKnowledgeDistillationLoss, self).__init__()
    ...
    def _calculate_filter_loss(self, student_model, teacher_model):
        ...
        student_attention = self._calculate_channel_attention(student_kernels)
        teacher_attention = self._calculate_channel_attention(teacher_kernels)
